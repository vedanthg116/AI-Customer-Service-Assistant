...
const AgentDashboard = () => {
  ...
  const [selectedConversation, setSelectedConversation] = useState(null); // NEW: Track full conversation details
  ...
  // NEW: Recorded Call Transcription States
  const [showTranscriptionModal, setShowTranscriptionModal] = useState(false);
  const [transcriptionText, setTranscriptionText] = useState('');
  const [transcriptionCustomerId, setTranscriptionCustomerId] = useState('');
  const [transcriptionCustomerName, setTranscriptionCustomerName] = useState('');
  ...
  const selectConversation = async (conversationId) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log(`AgentDashboard: Loaded ${data.length} messages for conversation ${conversationId}`);
        ...
        console.log('AgentDashboard: Found last customer message with analysis from history:', lastCustomerMessageWithAnalysis);
        
        if (lastCustomerMessageWithAnalysis) {
          // CRITICAL: Set AI suggestions from the analysis found in the fetched history
          console.log('AgentDashboard: Setting AI suggestions from fetched history (lastCustomerMessageWithAnalysis.analysis):', lastCustomerMessageWithAnalysis.analysis);
    ...
    await fetchConversationTickets(conversationId);
  };
  ...
  const handleSwitchAgent = () => {
    ...
    setShowTranscriptionModal(false); // Hide transcription modal if switching agent
  ...
  // NEW: Handle processing recorded call transcription
  const handleProcessRecordedCall = () => {
    setShowTranscriptionModal(true); // Show the transcription modal
    // Pre-fill with a new UUID for customer ID for easy testing, or leave blank
    setTranscriptionCustomerId(uuidv4());
    setTranscriptionCustomerName('');
    setTranscriptionText('');
  };

  // NEW: Handle submitting recorded call transcription
  const handleSubmitTranscription = async () => {
    if (transcriptionText.trim() === '' || transcriptionCustomerId.trim() === '' || transcriptionCustomerName.trim() === '') {
      alert("Please provide customer ID, customer name, and transcription text.");
      return;
    }

    setIsProcessingTranscription(true);
    try {
      const response = await fetch(`${API_BASE_URL}/process-recorded-call`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({
          customer_id: transcriptionCustomerId,
          customer_name: transcriptionCustomerName,
      ...
      if (response.ok) {
        const data = await response.json();
        console.log('AgentDashboard: Recorded call processed successfully:', data);
        alert('Recorded call transcription processed and sent to relevant conversation!');
        setShowTranscriptionModal(false); // Close modal
        setTranscriptionText('');
        setTranscriptionCustomerId('');
        setTranscriptionCustomerName('');
        fetchActiveConversations(); // Refresh conversation list to show new/updated conversation
      } else {
        const errorData = await response.json();
        console.error('AgentDashboard: Failed to process recorded call:', errorData.detail);
        alert(`Failed to process recorded call: ${errorData.detail}`);
      }
    } catch (error) {
      console.error('AgentDashboard: Network error processing recorded call:', error);
      alert("Network error or server unreachable while processing recorded call.");
    } finally {
  ...
  const handleAudioFileChange = (e) => {
    ...
    if (file) {
      ...
      if (!isValidType) {
        ...
        alert('Please select a valid audio file (WAV, MP3, M4A, FLAC, or OGG).');
        return;
      }
      ...
      setAudioFile(file);
      console.log('Audio file selected:', file.name, file.size, 'bytes', 'Type:', file.type);
    }
  };

  // NEW: Handle processing audio file
  const handleProcessAudioFile = async () => {
    if (!audioFile || transcriptionCustomerId.trim() === '' || transcriptionCustomerName.trim() === '') {
      alert("Please provide customer ID, customer name, and select an audio file.");
      return;
    }
    ...
    try {
      ...
      formData.append('customer_id', transcriptionCustomerId);
      formData.append('customer_name', transcriptionCustomerName);

      const response = await fetch(`${API_BASE_URL}/process-recorded-call-audio`, {
        method: 'POST',
        body: formData,
      });
      ...
      if (response.ok) {
        const data = await response.json();
        console.log('AgentDashboard: Audio file processed successfully:', data);
        ...
        alert('Audio file processed successfully! Transcription and analysis completed.');
        fetchActiveConversations(); // Refresh conversation list
      } else {
        const errorData = await response.json();
        console.error('AgentDashboard: Failed to process audio file:', errorData.detail);
        alert(`Failed to process audio file: ${errorData.detail}`);
      }
    } catch (error) {
      console.error('AgentDashboard: Network error processing audio file:', error);
      alert("Network error or server unreachable while processing audio file.");
    } finally {
      setIsProcessingAudio(false);
    }
  };

  // NEW: Handle marking ticket as fixed
  const handleMarkAsFixed = async (customer) => {
    if (!customer.latest_conversation_id) {
      alert("No conversation found for this customer.");
      return;
    }

    if (!customer.latest_ticket_issue) {
      alert("No ticket found for this customer.");
      return;
    }

    // Check if customer is assigned to another agent
    if (customer.assigned_agent_name && customer.assigned_agent_name !== agentName) {
      alert(`This customer is assigned to ${customer.assigned_agent_name}. Only the assigned agent can mark tickets as fixed.`);
      return;
    }

    try {
      const response = await fetch(`${API_BASE_URL}/mark-ticket-fixed`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({
          conversation_id: customer.latest_conversation_id,
          agent_id: agentId,
          agent_name: agentName,
          ticket_issue: customer.latest_ticket_issue,
      ...
      if (response.ok) {
        const data = await response.json();
        console.log('AgentDashboard: Ticket marked as fixed successfully:', data);
        alert('Ticket marked as fixed! A message has been sent to the customer.');
        // Refresh the customer overview data
        fetchCustomerOverview();
      } else {
        const errorData = await response.json();
        console.error('AgentDashboard: Failed to mark ticket as fixed:', errorData.detail);
        alert(`Failed to mark ticket as fixed: ${errorData.detail}`);
      }
    } catch (error) {
      console.error('AgentDashboard: Network error marking ticket as fixed:', error);
      alert("Network error or server unreachable while marking ticket as fixed.");
    }
  };
    ...
    <div className="panel agent-panel">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
        ...
        <div>
          ...
          <button onClick={handleProcessRecordedCall} className="home-button" style={{ marginRight: '10px' }}>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', marginTop: '15px' }}>
                <thead>
                  <tr style={{ backgroundColor: '#f2f2f2' }}>
                    ...
                    <th style={{ padding: '8px', border: '1px solid #ddd', textAlign: 'left' }}>Mark as Fixed</th>
                  </tr>
                </thead>
                <tbody>
                  {customerOverviewData.map((customer) => (
                    <tr key={customer.user_id} style={{ borderBottom: '1px solid #eee' }}>
                      ...
                      <td style={{ padding: '8px', border: '1px solid #ddd' }}>
                        {customer.latest_ticket_status === 'Open' && (
                          // Only show button if customer is unassigned OR assigned to current agent
                          (!customer.assigned_agent_name || customer.assigned_agent_name === agentName) ? (
                            <button onClick={() => handleMarkAsFixed(customer)} className="mark-fixed-button">Mark as Fixed</button>
                          ) : (
                            <span style={{ color: '#6c757d', fontSize: '0.8em' }}>
                              Assigned to {customer.assigned_agent_name}
                            </span>
                          )
                        )}
                      </td>
                  ...
                  {filteredConversations.map((conv) => (
                    ...
                    >
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                        ...
                        {conv.source === 'recorded_call' && (
                          <span className="recorded-call-badge">
                            📞 Recorded Call
                          </span>
                        )}
                      </div>
                ...
                <>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                      ...
                      {selectedConversation && selectedConversation.source === 'recorded_call' && (
                        <span className="recorded-call-badge-large">
                          📞 Recorded Call
                        </span>
                      )}
                    </div>
      ...
      {/* NEW: Recorded Call Transcription Modal */}
      {showTranscriptionModal && (
        <div className="modal-overlay">
          <div className="modal-content" style={{ maxWidth: '600px' }}>
            <h3>Process Recorded Call</h3>
            <div className="modal-body">
              <label>
                Customer ID: (e.g., from CRM, or auto-generated for new customer)
                <input
                  type="text"
                  value={transcriptionCustomerId}
                  onChange={(e) => setTranscriptionCustomerId(e.target.value)}
              ...
              <label>
                ...
                <input
                  type="text"
                  value={transcriptionCustomerName}
                  onChange={(e) => setTranscriptionCustomerName(e.target.value)}
              ...
              <div style={{ marginBottom: '20px', padding: '15px', backgroundColor: '#f8f9fa', borderRadius: '8px' }}>
                <h4 style={{ marginBottom: '10px', color: '#007bff' }}>🎤 Audio File Upload (Speech-to-Text)</h4>
                <div style={{ marginBottom: '10px', padding: '8px', backgroundColor: '#fff3cd', borderRadius: '4px', border: '1px solid #ffeaa7' }}>
                  <strong>💡 Tip:</strong> For best results, use <strong>WAV format (16-bit, 16kHz, mono)</strong>. 
                  Other formats may not work properly with Azure Speech Service.
                </div>
                <label>
                  Upload Audio File:
                  <input
                ...
                {audioFile && (
                  <div style={{ padding: '8px', backgroundColor: '#e9ecef', borderRadius: '4px', marginBottom: '10px' }}>
                    ...
                    {audioFile.name.toLowerCase().endsWith('.mp3') && (
                      <div style={{ color: '#856404', fontSize: '0.9em', marginTop: '5px' }}>
                        ⚠️ MP3 files may not work properly. Consider converting to WAV format.
                      </div>
                    )}
                  </div>
                )}
                <button 
                  onClick={handleProcessAudioFile} 
                  className="submit-button" 
                  disabled={isProcessingAudio || !audioFile || !transcriptionCustomerId.trim() || !transcriptionCustomerName.trim()}
                  style={{ width: '100%', marginBottom: '10px' }}
                >
                  {isProcessingAudio ? 'Processing Audio...' : 'Process Audio File'}
                </button>
              </div>
              ...
              {transcriptionResult && (
                <div style={{ marginBottom: '20px', padding: '15px', backgroundColor: '#d4edda', borderRadius: '8px', border: '1px solid #c3e6cb' }}>
                  <h4 style={{ marginBottom: '10px', color: '#155724' }}>✅ Transcription Result</h4>
                  <div style={{ marginBottom: '10px' }}>
                    <strong>Transcription:</strong>
                    <div style={{ padding: '8px', backgroundColor: 'white', borderRadius: '4px', marginTop: '5px', fontStyle: 'italic' }}>
                  ...
                  {transcriptionResult.analysis && (
                    <div>
                      ...
                      <div style={{ padding: '8px', backgroundColor: 'white', borderRadius: '4px', marginTop: '5px', fontSize: '0.9em' }}>
                        <div><strong>Intent:</strong> {transcriptionResult.analysis.predicted_intent || 'N/A'}</div>
                        <div><strong>Sentiment:</strong> {transcriptionResult.analysis.sentiment?.label || 'N/A'}</div>
                        {transcriptionResult.analysis.detected_entities && transcriptionResult.analysis.detected_entities.length > 0 && (
                          <div><strong>Entities:</strong> {transcriptionResult.analysis.detected_entities.map(e => e.entity).join(', ')}</div>
                        )}
                      </div>
                    </div>
                  )}
              ...
              <div style={{ borderTop: '2px solid #dee2e6', paddingTop: '15px' }}>
                <h4 style={{ marginBottom: '10px', color: '#6c757d' }}>📝 Manual Transcription (Alternative)</h4>
                <label>
                  Transcription Text:
                  <textarea
                    value={transcriptionText}
                    onChange={(e) => setTranscriptionText(e.target.value)}
                    placeholder="Paste the recorded call transcription here (if not using audio upload)..."
                    rows="6"
                    style={{ width: '100%', padding: '8px', borderRadius: '4px', border: '1px solid #ccc' }}
                  />
                </label>
            ...
            <div className="modal-footer" style={{ display: 'flex', justifyContent: 'flex-end', gap: '10px', marginTop: '20px' }}>
              <button 
                ...
                disabled={isProcessingTranscription || isProcessingAudio}
              >
                Close
              </button>
              <button 
                ...
                className="submit-button" 
                disabled={isProcessingTranscription || isProcessingAudio || (!transcriptionText.trim() && !transcriptionResult)}
              >
                {isProcessingTranscription ? 'Processing...' : 'Process Manual Transcription'}
              </button>
            </div>
          </div>
        </div>
